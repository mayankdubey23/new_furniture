...
const CartItemRow = ({ item, onRemove, onUpdateQuantity }) => (
  <div className="flex items-center gap-3 p-3 rounded-xl border border-theme-line">
    ...
    <div className="flex items-center gap-1 shrink-0">
      <button onClick={() => onUpdateQuantity(item.id, -1)} className="h-7 w-7 flex items-center justify-center rounded-full hover:bg-theme-sand/40 text-theme-walnut dark:text-theme-ink transition-colors">
        −
      </button>
      ...
      <button onClick={() => onRemove(item.id)} className="ml-1 h-7 w-7 flex items-center justify-center rounded-full hover:bg-red-50 text-theme-walnut/40 hover:text-red-500 transition-colors dark:text-theme-ink/40">
...
function Navbar() {
  ...
  const scrollToSection = useCallback((id) => {
    ...
    if (!target) return;
    if (lenis) {
      lenis.scrollTo(target, { offset: -140, duration: 1.15 });
    } else {
