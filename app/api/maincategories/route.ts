...
export async function GET(request: NextRequest) {
  try {
    ...
    const entities = await MainCategory.find(getActiveQuery(active)).sort({ active: -1, name: 1 }).lean();
    ...
    return NextResponse.json({ error: 'Failed to load main categories.' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const entity = await MainCategory.create(payload);
